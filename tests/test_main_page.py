...
class TestDropDownList:
    ...
    def test_questions_drop_down_list(self, driver, question, answer, true_answer):
        question_main_page = QuestionsAboutTheImportant(driver)
        question_main_page.scroll()
        question_main_page.wait_visibility_element(question)
        question_main_page.click_on_question(question)
        assert question_main_page.answer_text(answer) == true_answer
...
class TestNavigationOnSite:
    ...
    def test_go_to_main_page_click_scooter_logo(self, driver):
        ...
        assert navigation.get_current_url() == Urls.QA_SCOOTER
